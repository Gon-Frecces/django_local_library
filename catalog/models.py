...
class Genre(models.Model):    # all models must inherit from models.Model
...
class Book(models.Model):  # all models must inherit from models.Model
    """Model representing a book (but not a specific copy of a book)."""
    
    title = models.CharField(max_length=200)
    author = models.ForeignKey('Author', on_delete=models.RESTRICT, null=True)
    # Foreign key used because book can only have one author, but authors can have multiple books.
    # Author as a string rather than object because it hasnt been declared yet in file.
...
class BookInstance(models.Model):  # all models inherit from models.Model
    ...
    "particular book across the whole library")
